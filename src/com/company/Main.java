...
public class Main {
    ...
    public static void medicHeal() {
        ...
        if (isHavePacient) {
            ...
            while (!isSuccess) {
                int randomHero = new Random().nextInt(heroesHealth.length);

                if (heroesHealth[randomHero] > 0 && heroesHealth[randomHero] < 100 && heroesHealth[3] > 0) {
                    heroesHealth[randomHero] += heal;
                    System.out.println("medic choose " + heroesAttackType[randomHero] + " 50");
                    isSuccess = true;
                }
            }
        }
